  ...
  */
  describe('POST /scores', () => {
    it('it should approve loan when amount =< 10000', (done) => {
      chai.request(server)
        .post('/scores')
        .send({amount: 1000, user: {email: 'jane.doe@example.com', id: 1, name: 'jane'}, loans: []})
        .end((err, res) => {
    ...
    it('it should reject loan when amount > 10000', (done) => {
      chai.request(server)
        .post('/scores')
        .send({amount: 10001, user: {email: 'jane.doe@example.com', id: 1, name: 'jane'}, loans: []})
        .end((err, res) => {
          console.log(res.body)
          res.should.have.status(201);
          res.body.should.be.a('object');
  ...
  */
  describe('POST /scores', () => {
    ...
    it('it should return clear error message when loans is missing', (done) => {
      chai.request(server)
        .post('/scores')
        .send({amount: 1000, user: {email: 'jane.doe@example.com', id: 1, name: 'jane'}})
        .end((err, res) => {
          res.should.have.status(400);
          res.body.should.be.a('object');
  ...
  it('it should return clear error message when amount validation fails', (done) => {
    chai.request(server)
      .post('/scores')
      .send({amount: 100001, user: {email: 'jane.doe@example.com', id: 1, name: 'jane'}, loans: []})
      .end((err, res) => {
        res.should.have.status(400);
        res.body.should.be.a('object');
