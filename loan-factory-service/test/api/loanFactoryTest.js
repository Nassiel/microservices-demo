...
var scoringServicePost = nock('http://scoring-service:3000')
                          .post('/scores', {
                            amount:5000,
                            user: {id:1,name:'jane',email:'jane.doe@example.com'},loans:[{id:1,borrower_id:1,amount:1000,interest_rate:5,maturity:12}],approval:null,installmentPlan:null})
                          .reply(201, {
                            status: 'approved',
                            interest_rate: 6,
                            amount: 1000,
  ...
  */
  describe('POST /loans', () => {
    it('it should create loan and user when amount =< 10000', (done) => {
      chai.request(server)
        .post('/loans')
        .send({amount: 5000, maturity: 12, email: 'jane.doe@example.com'})
        .end((err, res) => {
